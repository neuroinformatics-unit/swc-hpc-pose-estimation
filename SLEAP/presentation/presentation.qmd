...
- Prepare a batch script, e.g. `slurm_train_script.sh`
- Submit the job:
  ```{.bash code-line-numbers=false}
...
```{.bash code-line-numbers="1-3|4-10|11-13"}
...
slurm_infer_script.sh
slurm_train_script.sh

# View the contents of the SLURM train script
cat slurm_train_script.sh
```

## Batch script for training {.smaller}

```{.bash filename="slurm_train_script.sh" code-line-numbers="1-12|13-24"}
#!/bin/bash
...
```{.bash filename="slurm_infer_script.sh" code-line-numbers="|25-33"}
#!/bin/bash
...
* So far we have submitted shell scripts
  * essentaiily using the command-line interface of SLEAP
...
```{.python filename="run_sleap_training" code-line-numbers="1-9|28-42"}
import argparse
...
```{.bash filename="slurm_train_script.sh" code-line-numbers="13-24"}
#!/bin/bash
...
* Laura Scwarz and Tom Hagley (SLEAP workflow)
